...
k_max = 200 * rad_um
Nk = 1000
T1_nrf, R1_nrf, T2_nrf, R2_nrf = scattering_problem_1d_nrf.T_and_R_really_all(omega_tab,
                                                              k_max, Nk)
A1_nrf = 1.0 - np.abs(T1_nrf)**2 - np.abs(R1_nrf)**2
A2_nrf = 1.0 - np.abs(T2_nrf)**2 - np.abs(R2_nrf)**2

T1_bare, R1_bare, T2_bare, R2_bare = scattering_problem_1d_multi.T_and_R_really_all_bare(omega_tab, k_max, Nk)

A1_bare = 1.0 - np.abs(T1_bare)**2 - np.abs(R1_bare)**2
A2_bare = 1.0 - np.abs(T2_bare)**2 - np.abs(R2_bare)**2
...
np.savez("exdip-scat-1d-L=%gnm-s=%gnm-B=%gmT-nrf.npz" % (res_L / nm,
                                                         ...
                                                         Bext  / mT),
             omega = omega_tab,
             T1_nrf = np.array(T1_nrf), R1_nrf = np.array(R1_nrf),
             A1_nrf = np.array(A1_nrf),
             T2_nrf = np.array(T2_nrf), R2_nrf = np.array(R2_nrf),
             A2_nrf = np.array(A2_nrf),
             T1_bare = np.array(T1_bare), R1_bare = np.array(R1_bare),
             A1_bare = np.array(A1_bare),
             T2_bare = np.array(T2_bare), R2_bare = np.array(R2_bare),
             A2_bare = np.array(A2_bare),
             #T1_nrf = np.array(T1_o_nrf), R1_nrf = np.array(R1_o_nrf),
             #A1_nrf = np.array(A1_o_nrf),
             #T2_nrf = np.array(T2_o_nrf), R2_nrf = np.array(R2_o_nrf),
             #A2_nrf = np.array(A2_o_nrf),
