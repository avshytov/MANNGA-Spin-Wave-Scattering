...
#omega_tab = np.linspace(0.9, 2.0, 1050 + 1) * GHz_2pi
...
k_max = 150 * rad_um
Nk = 1000
result_nrf = scattering_problem_1d_nrf.T_and_R_really_all(omega_tab,
                                                              k_max, Nk,
                                                              True)
T1_nrf, R1_nrf, T2_nrf, R2_nrf, phi_R_nrf, phibar_R_nrf, phi_L_nrf, phibar_L_nrf = result_nrf
A1_nrf = 1.0 - np.abs(T1_nrf)**2 - np.abs(R1_nrf)**2
A2_nrf = 1.0 - np.abs(T2_nrf)**2 - np.abs(R2_nrf)**2
...
np.savez("exdip-scat-1d-L=%gnm-s=%gnm-B=%gmT-nrf.npz" % (res_L / nm,
             ...
             phi_R_nrf  = np.array(phi_R_nrf),
             phibar_R_nrf = np.array(phibar_R_nrf), 
             phi_L_nrf    = np.array(phi_L_nrf),
             phibar_L_nrf = np.array(phibar_L_nrf),
