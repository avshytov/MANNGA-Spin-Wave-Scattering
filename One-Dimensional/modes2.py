...
class Mode:
    ...
    def normalize(self, area = ''):
        ...
        if np.abs(C_norm) > 1e-16:
           self.scale(1.0 / np.sqrt(np.abs(C_norm)))
        else:
           norm_mxz  = linalg.norm(self.mx_all) + linalg.norm(self.mz_all)
           norm_dual = linalg.norm(self.mx_dual_all) + linalg.norm(self.mz_dual_all)
           scale_factor = np.sqrt(norm_dual / norm_mxz)
...
class CellArrayModel:
    ...
    def get_I_alpha(self):
        ...
        I[1::2, 0::2] =   np.diag(I_zx)
        I[0::2, 1::2] = - np.diag(I_zx)
        I[0::2, 0::2] =    -np.diag(I_zx_alpha)
        I[1::2, 1::2] =    -np.diag(I_zx_alpha)
        return I
