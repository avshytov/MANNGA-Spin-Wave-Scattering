...
def readData(fname):
    ...
    res_min = 1.85 * GHz_2pi
    res_max = 2.05 * GHz_2pi

    Omega_3, Gamma_R3, Gamma_L3, Gamma_3, Gamma_tot3 = resfit.fit_T_and_T(
         res_min, res_max, omega, T1, T2)
    ...
    pl.figure()
    pl.plot(omega/ GHz_2pi, np.abs(T1), label='T1')
    pl.plot(omega/ GHz_2pi, np.abs(R1), label='R1')
    pl.plot(omega/ GHz_2pi, np.abs(T2), label='T2')
    pl.plot(omega/ GHz_2pi, np.abs(R2), label='R2')
