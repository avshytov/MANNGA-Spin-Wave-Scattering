...
def readData(fname):
    ...
    show_phi = False
    if 'phi_R_nrf' in d.keys():
        show_phi = True
        phi_R_bare = d['phi_R_bare']
        phibar_R_bare = d['phibar_R_bare']
        phi_L_bare = d['phi_L_bare']
        phibar_L_bare = d['phibar_L_bare']
        phi_R_nrf = d['phi_R_nrf']
        phibar_R_nrf = d['phibar_R_nrf']
        phi_L_nrf = d['phi_L_nrf']
        phibar_L_nrf = d['phibar_L_nrf']
    ...
    if show_phi:
        ...
        Nphi, Mphi = np.shape(phi_R_nrf)
        if len(mode_freq) < 1:
            mode_freq = np.array(range(0, Mphi))
        print ("mode_freq = ", mode_freq, np.shape(mode_freq))
        print ("omega: ", np.shape(omega))
        print("phi_R = ", np.shape(phi_R_nrf))
        Y, X = np.meshgrid(mode_freq.real / GHz_2pi, omega / GHz_2pi)
        from matplotlib.colors import LogNorm
        pl.figure()
        pl.pcolormesh(X, Y, np.abs(phi_R_nrf[:-1, :-1]), shading='auto',      
