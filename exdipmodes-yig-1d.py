...
res_bias = 5 * mT
res_alpha = 0.01
res_Nx = 0.162 #np.pi / 2.0 * res_H / res_L
res_Ny = 1.0 - res_Nx
#res_Nx = 0.5
#res_Ny = 0.5
res_theta = 0.0
...
from scattering import ScatteringProblem_1D
        
scattering_problem_1d = ScatteringProblem_1D(slab, resonator, z_res)
...
def solve_1d(omega_tab):
    ...
    for omega in omega_tab:
        Gamma_rad = scattering_problem_1d.Gamma_rad(omega)
        Gamma_rad_o.append(Gamma_rad)
        T1, R1, T2, R2 = scattering_problem_1d.T_and_R(omega)
        T1_o.append(T1)
        R1_o.append(R1)
        ...
        A2_o.append(A2)
        print ("o = ", omega, "T, R, A = ", np.abs(T1), np.abs(R1), A1)
    np.savez("exdip-scat-1d.npz", omega = omega_tab,
             ...
             **scattering_problem_1d.describe())    

#omega_tab = np.linspace(2.0, 3.0, 50 + 1) * GHz_2pi
omega_tab = np.linspace(1.0, 4.0, 300 + 1) * GHz_2pi
solve_1d(omega_tab)
