...
class Mode:
    def __init__ (self, model, f, mode_mxz):
        self.f = f
        self.model  = model
    ...
    def copy(self):
        mxz = np.zeros((len(self.mx_all) + len(self.mz_all)), dtype=self.mx_all.dtype)
        mxz[0::2] = self.mx_all
        mxz[1::2] = self.mz_all
        mode_copy = Mode(self.model, self.f, mxz)
...
class CellArrayModel:
    ...
    def solve(self):
        ...
        modes_unpacked = []
        for t in modes_pos:
            modes_unpacked.append(Mode(self, f[t], mxz[:, t]))
